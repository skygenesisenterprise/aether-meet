    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Link
          href={href}
          className={cn(
            "group relative flex h-[57px] flex-col items-center justify-center gap-[3px] text-[#b6b7ba] transition-colors hover:bg-white/[0.035] hover:text-white",
            active && "text-[#8b89ff]"
          )}
          aria-current={active ? "page" : undefined}
        >
          {active ? (
            <span className="absolute left-0 h-10 w-[2px] rounded-r-full bg-[#7775ff]" />
          ) : null}
          <span
            className={cn(
          ...
          <span className="max-w-[64px] truncate px-0.5 text-[9px] leading-3">{label}</span>
        </Link>
      </TooltipTrigger>
...
export function AdminSidebar() {
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <aside className="hidden h-full w-[68px] shrink-0 flex-col border-r border-white/[0.06] bg-[#0d0e10] text-zinc-300 md:flex">
        ...
        <div className="border-t border-white/[0.05] pb-1 pt-0.5">
          <button
            type="button"
          ...
          <button
            type="button"
            className="flex h-[52px] w-full flex-col items-center justify-center gap-1 text-[#b6b7ba] transition-colors hover:bg-white/[0.035] hover:text-white"
            aria-label="Applications"
          >
            <span className="flex size-7 items-center justify-center rounded-[5px] border border-current">
              <AppWindow className="size-[17px]" strokeWidth={1.7} />
            </span>
