import {
  ChevronDown,
  Clock3,
  FilePenLine,
  FileText,
  MoreHorizontal,
  ShieldCheck,
  Sparkles,
  Star,
} from "lucide-react";

import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

const documents = [
  {
    id: "doc-1",
    title: "Plan de lancement Aether Meet",
    context: "Produit · Mis à jour il y a 14 min",
    status: "En revue",
    accent: "bg-amber-500/15 text-amber-300 border-amber-400/20",
  },
  {
    id: "doc-2",
    title: "Compte-rendu comité sécurité",
    context: "Conformité · Modifié aujourd’hui, 10:12",
    status: "Validé",
    accent: "bg-emerald-500/15 text-emerald-300 border-emerald-400/20",
  },
  {
    id: "doc-3",
    title: "Brief expérience appels",
    context: "Design system · Hier, 18:04",
    status: "Brouillon",
    accent: "bg-sky-500/15 text-sky-300 border-sky-400/20",
  },
  {
    id: "doc-4",
    title: "Synthèse retours clients S2",
    context: "Customer success · Hier, 15:28",
    status: "Partagé",
    accent: "bg-violet-500/15 text-violet-300 border-violet-400/20",
  },
] as const;

const filters = ["Récents", "En revue", "Brouillons", "Partagés", "Archivés"] as const;

const collections = [
  {
    title: "Notes de réunion",
    description: "12 documents actifs",
    icon: Clock3,
  },
  {
    title: "Documents sensibles",
    description: "Politiques, sécurité et revue légale",
    icon: ShieldCheck,
  },
  {
    title: "Templates IA",
    description: "Prompts, résumés et structures réutilisables",
    icon: Sparkles,
  },
] as const;

export default function DocumentsPage() {
  return (
    <div className="flex h-full min-h-180 flex-col bg-[#202123]">
      <header className="flex min-h-15.5 shrink-0 items-center justify-between border-b border-white/12 bg-[#202123] px-5">
        <div className="flex h-full items-center gap-5">
          <div className="flex items-center gap-3">
            <span className="flex size-8 items-center justify-center rounded-sm bg-[#5d5bd4] text-white">
              <FileText className="size-4.5" />
            </span>
            <h1 className="text-lg font-semibold">Documents</h1>
          </div>
          <button
            type="button"
            className="relative flex h-full items-center px-1 text-sm font-semibold"
          >
            Mes documents
            <span className="absolute inset-x-0 bottom-0 h-0.5 rounded-full bg-[#7775ff]" />
          </button>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="rounded-sm">
            Nouveau document
            <FilePenLine className="size-4" />
          </Button>
          <Button variant="ghost" size="icon-sm" className="rounded-sm" aria-label="Documents favoris">
            <Star className="size-4 text-[#8b89ff]" />
          </Button>
        </div>
      </header>

      <div className="grid min-h-0 flex-1 grid-cols-1 xl:grid-cols-[minmax(520px,1fr)_minmax(300px,0.72fr)]">
        <section className="flex min-h-0 flex-col border-r border-white/12">
          <div className="flex min-h-12.5 flex-wrap items-center gap-2 border-b border-white/12 px-5 py-2">
            <button type="button" className="flex items-center gap-2 text-sm font-semibold">
              <ChevronDown className="size-3.5" />
              Travail actif
            </button>
            <div className="ml-auto flex flex-wrap items-center gap-2">
              {filters.map((filter, index) => (
                <button
                  key={filter}
                  type="button"
                  className={cn(
                    "rounded-full border border-zinc-600 px-3 py-1 text-sm text-zinc-200 transition-colors hover:bg-white/5",
                    index === 0 && "border-[#7775ff] bg-[#7775ff] font-semibold text-white"
                  )}
                >
                  {filter}
                </button>
              ))}
              <Button variant="ghost" size="icon-sm" className="rounded-sm" aria-label="Plus d’options">
                <MoreHorizontal className="size-4" />
              </Button>
            </div>
          </div>

          <div className="min-h-0 flex-1 overflow-auto">
            {documents.map((document) => (
              <article
                key={document.id}
                className="group flex items-center gap-3 border-b border-white/7 px-5 py-3 transition-colors hover:bg-white/2.5"
              >
                <span className="flex size-9 shrink-0 items-center justify-center rounded-sm bg-white/5 text-zinc-200">
                  <FileText className="size-4" />
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">{document.title}</p>
                  <p className="mt-0.5 truncate text-xs text-zinc-500">{document.context}</p>
                </div>
                <span
                  className={cn(
                    "rounded-full border px-2.5 py-1 text-xs font-medium",
                    document.accent
                  )}
                >
                  {document.status}
                </span>
              </article>
            ))}
          </div>
        </section>

        <aside className="hidden min-h-0 flex-col xl:flex">
          <div className="flex h-12.5 shrink-0 items-center justify-between border-b border-white/12 px-5">
            <h2 className="text-base font-semibold">Collections</h2>
            <Button variant="ghost" size="icon-sm" className="rounded-sm" aria-label="Gérer les collections">
              <MoreHorizontal className="size-4" />
            </Button>
          </div>
          <div className="space-y-3 p-5">
            {collections.map((collection) => {
              const Icon = collection.icon;

              return (
                <button
                  key={collection.title}
                  type="button"
                  className="flex w-full items-start gap-3 rounded-sm border border-white/8 bg-white/2 p-3 text-left transition-colors hover:bg-white/5"
                >
                  <span className="mt-0.5 flex size-8 items-center justify-center rounded-sm bg-white/5 text-zinc-200">
                    <Icon className="size-4" />
                  </span>
                  <span className="min-w-0">
                    <span className="block text-sm font-medium">{collection.title}</span>
                    <span className="mt-0.5 block text-xs text-zinc-500">{collection.description}</span>
                  </span>
                </button>
              );
            })}
          </div>
        </aside>
      </div>
    </div>
  );
}
